export default class Person {
  ...
  static getAge(date: Date): number { // copiei do course pra ganhar tempo
  ...
  private validateName(name: string): void {
    if (name.length < this.MIN_NAME_LENGTH) {
      throw new Error(`O nome deve conter no mínimo ${this.MIN_NAME_LENGTH} caracteres.`);
    }
  }
  ...
  private validateBirthDate(date: Date): void {
    ...
    if (Person.getAge(date) > this.MAX_AGE) {
      throw new Error(`A pessoa deve ter no máximo ${this.MAX_AGE} anos.`);
    }
  }
